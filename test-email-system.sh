...
if [[ "$generate_attack" =~ ^[Yy]$ ]]; then
  ...
  COWRIE_LOG_PATH=$(echo "$EMAIL_DEBUG" | jq -r '.environment.COWRIE_LOG_PATH' 2>/dev/null)
  
  if [ -z "$COWRIE_LOG_PATH" ] || [ "$COWRIE_LOG_PATH" = "null" ] || [ "$COWRIE_LOG_PATH" = "Not set" ]; then
    echo -e "${RED}Error: COWRIE_LOG_PATH is not set or invalid.${NC}"
    echo -e "${YELLOW}Please run ./fix-email-alerts.sh to set the correct log path.${NC}"
  else
...
echo "1. Run ./fix-email-alerts.sh to reconfigure your email settings"
